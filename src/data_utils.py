...
senttag2word = {'POS': 'positive', 'NEG': 'negative', 'NEU': 'neutral', 'pos': 'positive', 'neg': 'negative', 'neu': 'neutral'}
senttag2opinion = {'POS': 'great', 'NEG': 'bad', 'NEU': 'ok', 'pos': 'great', 'neg': 'bad', 'neu': 'ok'}
...
def get_transformed_io(data_path, data_name, data_type, top_k, args):
    ...
    print(f"read_line_examples_from_file 结果：sents长度={len(sents)}, labels长度={len(labels)}")  # 关键！
    ...
    if data_type == "train" or args.eval_data_split == "dev" or data_type == "test":
        ...
        new_inputs, targets = get_para_targets_dev(inputs, labels, data_name,
                                                   args.task, args)
    print(f"get_para_targets结果：new_inputs长度={len(new_inputs)}, targets长度={len(targets)}")  # 关键！
...
class ABSADataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        aspect_sentiment_label = torch.tensor(self.contrastive_labels['aspect_sentiment'][index])
        return {"source_ids": source_ids,
                "source_mask": src_mask,
                "target_ids": target_ids,
                "target_mask": target_mask,
                'sentiment_labels': sentiment_label,
                'aspect_sentiment_labels': aspect_sentiment_label,
                }

    def get_raw_labels(self):
        results = get_transformed_io(self.data_path, self.data_dir, self.task, self.data_type)
        return results
    ...
    def _build_examples(self):
        ...
        print(f"inputs 总长度: {len(inputs)}")
        print(f"inputs 前3个样本: {inputs[:3]}")
        print(f"inputs 第1个样本的类型: {type(inputs[0]) if len(inputs) > 0 else '无'}")
        if len(inputs) > 0:
            print(
                f"inputs 第1个样本的长度: {len(inputs[0]) if isinstance(inputs[0], (list, tuple)) else '单字符串无长度'}")
        print("=" * 50)


        if not self.args.multi_task and len(labels) > 0:
            ...
            def get_sentiment_labels(labels_in):
                ...
                for ex in labels_in:
                    ...
                    label_list = list(set([quad[2] for quad in ex]))
                    label_str = label_list[0]
                    label = sentiment_dict[label_str]
                    assert label in [0, 1, 2]
                    sentiment_labels.append(label)
                return sentiment_labels


            def get_aspect_sentiment_labels(labels_in):
                ...
                for ex in labels_in:
                    for quad in ex:
                        ...
                        aspect = quad[0] if isinstance(quad[0], (str, tuple)) else tuple(quad[0])
                        sentiment = quad[2] if isinstance(quad[2], (str, tuple)) else tuple(quad[2])
                        sa_pair = (aspect, sentiment)

                        if sa_pair not in sa_pair_to_id:
                            sa_pair_to_id[sa_pair] = next_id
                            next_id += 1
                ...
                for ex in labels_in:
                    ...
                    aspect_sentiment_labels.append(sa_pair_to_id[sa_pair])

                return aspect_sentiment_labels

            raw_sentiment = get_sentiment_labels(labels)

            raw_aspect_sentiment = get_aspect_sentiment_labels(labels)

            n = len(inputs) // len(labels)

            expanded_sentiment = [label for label in raw_sentiment for _ in range(n)]
            expanded_aspect_sentiment = [label for label in raw_aspect_sentiment for _ in range(n)]

            self.contrastive_labels['sentiment'] = expanded_sentiment
            self.contrastive_labels['aspect_sentiment'] = expanded_aspect_sentiment
        else:
            self.contrastive_labels = {
                'sentiment': [],
                'aspect_sentiment': []
        ...
        for i in range(len(inputs)):
            # 转换为字符串
            input_str = ' '.join(inputs[i])
            target_str = targets[i]

            tokenized_input = self.tokenizer.batch_encode_plus(
                [input_str], max_length=self.max_len, padding="max_length",
                truncation=True, return_tensors="pt")

            target_max_length = 1024 if self.data_type == "test" else self.max_len
            tokenized_target = self.tokenizer.batch_encode_plus(
                [target_str], max_length=target_max_length, padding="max_length",
                truncation=True, return_tensors="pt")
        ...
        # 4. 打印调试信息 (验证数据正确性)
        print(f"样本总数: {len(self.inputs)}")
        if not self.args.multi_task and len(labels) > 0:
            print(f"sentiment标签数量: {len(self.contrastive_labels['sentiment'])}")
            print(f"aspect_sentiment (A+S) 标签数量: {len(self.contrastive_labels['aspect_sentiment'])}")
