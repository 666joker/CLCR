...
from eval_utils import compute_scores, extract_spans_para, compute_semantic_f1
...
def init_args():
    ...
    parser.add_argument(
        "--task",
        default='aste',
        choices=["aste", "tasd", "unified", "unified3"],
        type=str,
        help="The name of the task, selected from: [asqp, aste]")
    parser.add_argument(
        "--dataset",
        default='rest15',
        type=str,
    ...
    parser.add_argument('--agg_strategy', type=str, default='vote', choices=['vote', 'rand', 'heuristic', 'consensus'])
...
class T5FineTuner(pl.LightningModule):
    ...
    def __init__(self, config, tfm_model, tokenizer, cont_model, cont_as_model):
        super().__init__()
        self.save_hyperparameters(ignore=['tfm_model', 'tokenizer', 'cont_model', 'cont_as_model'])
        self.config = config
        self.model = tfm_model
        self.tokenizer = tokenizer
        self.automatic_optimization = False
        self.cont_model = cont_model
        self.cont_as_model = cont_as_model
    ...
    def forward(
        ...
        main_pred = self.model(
            ...
            labels=labels,
            output_hidden_states=True  # 需保留以获取用于对比学习的隐藏状态
        )
        ...
        cont_as_pred = self.cont_as_model(last_state, attention_mask)
        ...
        return (
            ...
            cont_pred,
            cont_as_pred,
            pooled_encoder_layer
        )
    def _step(self, batch):
        lm_labels = batch["target_ids"]
        lm_labels[lm_labels[:, :] == self.tokenizer.pad_token_id] = -100

        outputs, cont_pred, cont_as_pred, pooled_encoder_layer = self(
        ...
        criterion = SupConLoss(loss_scaling_factor=self.config.cont_loss, temperature=self.config.cont_temp)
        sentiment_labels = batch['sentiment_labels']
        aspect_sentiment_labels = batch['aspect_sentiment_labels']
        ...
        cont_as_summed = cont_as_pred
        cont_as_snormed = normalize(cont_as_summed, p=2.0, dim=2)
        aspect_sentiment_contrastive_loss = criterion(cont_as_snormed, aspect_sentiment_labels)

        sentiment_encs = cont_normed.detach().cpu().numpy()[:, 0].tolist()
        aspect_sentiment_encs = cont_as_snormed.detach().cpu().numpy()[:, 0].tolist()

        sentiment_labs = sentiment_labels.detach().cpu().tolist()
        aspect_sentiment_labs = aspect_sentiment_labels.detach().cpu().tolist()


        # return original loss plus the characteristic-specific SCL losses

        loss = outputs[0]*100 + sentiment_contrastive_loss +  aspect_sentiment_contrastive_loss

        # 如果后续需要调整损失平衡（如生成任务更重要），可修改outputs[0]的系数（如 ×200）；
        return loss
    ...
    def training_step(self, batch, batch_idx):
        ...
        # 4. 手动梯度裁剪（替代原gradient_clip_val=1.0）
        torch.nn.utils.clip_grad_norm_(
            self.model.parameters(),  # 裁剪模型所有参数的梯度
            max_norm=1.0  # 与原配置保持一致的裁剪阈值
        )
    ...
    def evaluate(self, batch, stage=None):
        # get f1  if not f1 and pre and rec
        ...
        # precision = torch.tensor(scores['pre'], dtype=torch.float64)
        # recall = torch.tensor(scores['rec'], dtype=torch.float64)
    ...
    def configure_optimizers(self):
        ...
        cont_as_model = self.cont_as_model
        ...
        optimizer_grouped_parameters = [
            ...
            {
                "params": [p for n, p in cont_as_model.named_parameters() if not any(nd in n for nd in no_decay)],
                "weight_decay": self.config.weight_decay,
            }, {
                "params": [p for n, p in cont_as_model.named_parameters() if any(nd in n for nd in no_decay)],
                "weight_decay": 0.0,
            }
        ]
    ...
    if args.multi_path:
        ...
        for i in range(0, len(targets)):
            ...
            if args.agg_strategy == 'post_rank':
                ...
                continue
            elif args.agg_strategy == 'consensus':
                if len(multi_outputs) <= 1:
                    outputs.append(multi_outputs[0] if multi_outputs else "")
                    continue

                consensus_scores = []
                for i_cand in multi_outputs:
                    current_score = 0
                    for j_cand in multi_outputs:
                        if i_cand == j_cand:
                            continue

                        sim_scores, _, _ = compute_scores([i_cand], [j_cand], verbose=False)
                        current_score += sim_scores['f1']
                    consensus_scores.append(current_score)

                best_idx = np.argmax(consensus_scores)
                outputs.append(multi_outputs[best_idx])
                continue
...
def train_function(args):
    ...
    if args.do_train:
        ...
        cont_as_model = LinearModel()

        model = T5FineTuner(args, tfm_model, tokenizer, cont_model,cont_as_model)
